  ...
  if (errorResultXdr) {
    return _objectSpread(_objectSpread(_objectSpread({}, r), diagnosticEventsXdr !== undefined && diagnosticEventsXdr.length > 0 && {
      diagnosticEvents: diagnosticEventsXdr.map(function (evt) {
        return _diamnetBase.xdr.DiagnosticEvent.fromXDR(evt, 'base64');
      })
    }), {}, {
      errorResult: _diamnetBase.xdr.TransactionResult.fromXDR(errorResultXdr, 'base64')
    });
  }
  ...
  return {
    ...
    events: ((_r$events = r.events) !== null && _r$events !== void 0 ? _r$events : []).map(function (evt) {
      ...
      return _objectSpread(_objectSpread(_objectSpread({}, clone), evt.contractId !== '' && {
        contractId: new _diamnetBase.Contract(evt.contractId)
      }), {}, {
        topic: evt.topic.map(function (topic) {
          return _diamnetBase.xdr.ScVal.fromXDR(topic, 'base64');
        }),
        value: _diamnetBase.xdr.ScVal.fromXDR(evt.value, 'base64')
      });
    })
  };
}
  ...
  return {
    ...
    entries: ((_raw$entries = raw.entries) !== null && _raw$entries !== void 0 ? _raw$entries : []).map(function (rawEntry) {
      ...
      return _objectSpread({
        ...
        key: _diamnetBase.xdr.LedgerKey.fromXDR(rawEntry.key, 'base64'),
        val: _diamnetBase.xdr.LedgerEntryData.fromXDR(rawEntry.xdr, 'base64')
...
function parseSuccessful(sim, partial) {
  ...
  var success = _objectSpread(_objectSpread(_objectSpread({}, partial), {}, {
    transactionData: new _diamnetBase.SorobanDataBuilder(sim.transactionData),
    ...
    result: sim.results.map(function (row) {
      ...
      return {
        auth: ((_row$auth = row.auth) !== null && _row$auth !== void 0 ? _row$auth : []).map(function (entry) {
          return _diamnetBase.xdr.SorobanAuthorizationEntry.fromXDR(entry, 'base64');
        }),
        retval: row.xdr ? _diamnetBase.xdr.ScVal.fromXDR(row.xdr, 'base64') : _diamnetBase.xdr.ScVal.scvVoid()
      };
    })[0]
    ...
    stateChanges: (_sim$stateChanges2 = sim.stateChanges) === null || _sim$stateChanges2 === void 0 ? void 0 : _sim$stateChanges2.map(function (entryChange) {
      return {
        ...
        key: _diamnetBase.xdr.LedgerKey.fromXDR(entryChange.key, 'base64'),
        before: entryChange.before ? _diamnetBase.xdr.LedgerEntry.fromXDR(entryChange.before, 'base64') : null,
        after: entryChange.after ? _diamnetBase.xdr.LedgerEntry.fromXDR(entryChange.after, 'base64') : null
      };
    })
  });
  ...
  return _objectSpread(_objectSpread({}, success), {}, {
    restorePreamble: {
      ...
      transactionData: new _diamnetBase.SorobanDataBuilder(sim.restorePreamble.transactionData)
    }
  });
}
