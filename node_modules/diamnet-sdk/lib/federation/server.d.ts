...
export declare class FederationServer {
     ...
     *
     * * For Diamante address (ex. `bob*diamcircle.io`) it splits Diamante address and then tries to find information about
     * federation server in `diamante.toml` file for a given domain. It returns a `Promise` which resolves if federation
     ...
     * Example:
     * ```js
     * DiamSdk.FederationServer.resolve('bob*diamcircle.io')
     ...
     *
     * If `diamante.toml` file does not exist for a given domain or it does not
     * contain information about a federation server Promise will reject.
     * ```js
     * DiamSdk.FederationServer.createForDomain('acme.com')
     *   .then(federationServer => {
     *     // federationServer.resolveAddress('bob').then(...)
     *   })
     *   .catch(error => {
     *     // diamante.toml does not exist or it does not contain information about federation server.
     *   });
     * ```
     ...
     * @param {string} address Stellar address (ex. `bob*diamcircle.io`). If `FederationServer` was instantiated with `domain` param only username (ex. `bob`) can be passed.
