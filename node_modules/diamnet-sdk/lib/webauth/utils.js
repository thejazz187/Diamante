...
function buildChallengeTx(serverKeypair, clientAccountID, homeDomain) {
  ...
  var account = new _diamnetBase.Account(serverKeypair.publicKey(), "-1");
  var now = Math.floor(Date.now() / 1000);
  var value = (0, _randombytes.default)(48).toString("base64");
  var builder = new _diamnetBase.TransactionBuilder(account, {
    fee: _diamnetBase.BASE_FEE,
    networkPassphrase: networkPassphrase,
    timebounds: {
      minTime: now,
      maxTime: now + timeout
    }
  }).addOperation(_diamnetBase.Operation.manageData({
  ...
  if (clientDomain) {
    ...
    builder.addOperation(_diamnetBase.Operation.manageData({
      name: "client_domain",
      value: clientDomain,
  ...
  if (memo) {
    builder.addMemo(_diamnetBase.Memo.id(memo));
  }
...
function readChallengeTx(challengeTx, serverAccountID, networkPassphrase, homeDomains, webAuthDomain) {
  ...
  var transaction;
  try {
    transaction = new _diamnetBase.Transaction(challengeTx, networkPassphrase);
  } catch (_unused) {
    try {
      transaction = new _diamnetBase.FeeBumpTransaction(challengeTx, networkPassphrase);
    } catch (_unused2) {
  ...
  if (transaction.timeBounds && Number.parseInt((_transaction$timeBoun = transaction.timeBounds) === null || _transaction$timeBoun === void 0 ? void 0 : _transaction$timeBoun.maxTime, 10) === _diamnetBase.TimeoutInfinite) {
...
function verifyChallengeTxSigners(challengeTx, serverAccountID, networkPassphrase, signers, homeDomains, webAuthDomain) {
  ...
  var serverKP;
  try {
    serverKP = _diamnetBase.Keypair.fromPublicKey(serverAccountID);
  } catch (err) {
...
function gatherTxSigners(transaction, signers) {
  ...
  try {
    for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
      ...
      var keypair = void 0;
      try {
        keypair = _diamnetBase.Keypair.fromPublicKey(signer);
      } catch (err) {
